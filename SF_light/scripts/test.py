...
def occupancyByZone(isTAZ):
	global paths, links, vehicles, vehicleTypes, nodes
	def processNodes (**kwargs):
		'''kwargs: polys, nodes, st, end'''
		nodes = kwargs.get("nodes")
		polys = kwargs.get("polys")
...
def tripsByTime():
	...
	for i in range(NUM_THREADS):
		...
		threads.append(processingThread(i, "trip_thread_" + str(i), processTrips, linkMap=linkMap, legMap=legMap, trips=trips, st=st, end=end))
	for thread in threads:
		thread.start()
	for thread in threads:
		thread.join()
...
def followPeople():
	...
	for i in range(len(activities["PID"])):
		...
		destinationAct = trips["DestinationAct"][i]
